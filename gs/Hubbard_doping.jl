...
ctm_alg_AD = SequentialCTMRG(; tol=1e-10, maxiter=30, verbosity=3);
ctm_alg_SU = SequentialCTMRG(; tol=1e-10, maxiter=30, verbosity=3, trscheme = truncdim(χenv));
opt_alg = PEPSOptimize(;
    boundary_alg=ctm_alg_AD,
    optimizer_alg=LBFGS(10; maxiter=10, gradtol=1e-8, verbosity=3),
    gradient_alg=LinSolver(; iterscheme=:diffgauge),
    reuse_env=true,
);

filename_save = "Hubbard_testing"
AD_steps = 2
iterations = 2
H_simple = ham
...
for iter = 1:iterations
    ...
    custom_finalize! = (x, f, g, numiter) -> custom_finalize_function(x, f, g, numiter, filename_save * "_$(iter).jld2")
    global ψ
    global env
